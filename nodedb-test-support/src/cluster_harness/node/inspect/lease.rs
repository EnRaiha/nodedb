...
impl TestClusterNode {
    ...
    /// an active drain entry (installed and not yet explicitly
    /// ended) for the given `(descriptor_id, min_version)`. Used
    /// by the drain tests to assert replicated drain state.
