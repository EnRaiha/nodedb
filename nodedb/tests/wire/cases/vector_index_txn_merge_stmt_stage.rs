...
//! In-transaction `MERGE` is resolved + staged at STATEMENT time, not
//! expanded at COMMIT: a later `SELECT` in the same transaction sees the
//! merge-inserted row (read-your-own-writes), and a later `UPDATE` in the
//! same transaction resolves against an overlay that already holds the
//! merge's post-image, moving the SAME surrogate the MERGE created.
