...
//! Live (no-restart) coverage for the secondary vector index on an
//! autocommit `UPDATE`: the new embedding must become searchable and the
//! pre-update embedding must be removed from the HNSW index, same process.
