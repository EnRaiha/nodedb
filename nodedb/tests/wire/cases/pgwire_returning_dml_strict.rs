...
//! `RETURNING` over `WITH (engine='document_strict')` collections: the
//! returned `id` must be the declared primary key, not the surrogate
//! storage key, and pre-image columns must be the real stored values.
