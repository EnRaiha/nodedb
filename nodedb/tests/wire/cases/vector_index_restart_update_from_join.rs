...
//! Restart durability for a secondary vector index under a joined
//! `UPDATE target SET ... FROM source`: the new embedding must survive a
//! WAL-only restart, and the pre-update vector must not resurrect.
