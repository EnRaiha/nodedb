...
//! predicate (bulk) `UPDATE` — a non-PK filter routes to
//! `execute_bulk_update`, and its new embedding must reindex into the HNSW,
//! not leave the pre-update vector stuck under the surrogate.
