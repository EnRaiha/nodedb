...
//! A table-qualified WHERE column (`u.name` / `users.name`) on a
//! single-table query must not silently match zero rows: the qualifier
//! must not leak into the scan filter's field name.
