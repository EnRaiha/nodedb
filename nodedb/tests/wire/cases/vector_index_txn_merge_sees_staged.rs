...
//! In-transaction visibility of the MERGE / `UPDATE ... FROM` RESOLVE pass:
//! a statement resolved at COMMIT must see rows staged by EARLIER
//! statements in the SAME transaction (read-your-own-writes for the
//! resolve). The resolve scan folds the transaction's staging overlay into
//! its scan, so it sees `base ∪ overlay` rather than committed base alone.
