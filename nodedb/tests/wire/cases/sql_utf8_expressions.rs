...
//! UTF-8 correctness in expression parsing: the `nodedb-query` tokenizer
//! slices `&str` byte-by-byte, so multi-byte codepoints in generated
//! columns, check constraints, and typeguard expressions must not panic.
