...
//! Restart durability for a secondary vector index under `TRUNCATE`: a
//! truncated row's vector must not resurrect in the WAL-rebuilt HNSW, and
//! rows inserted after the truncate must remain searchable.
