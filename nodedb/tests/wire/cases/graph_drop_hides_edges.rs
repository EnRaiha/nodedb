...
//! A plain (soft) `DROP COLLECTION` must hide a graph collection's edges and
//! stats from `MATCH` and `SHOW GRAPH STATS` (single and tenant-wide), and
//! `UNDROP` must restore them. `DROP ... PURGE` removes them outright.
