...
//! A row inserted by `MERGE ... WHEN NOT MATCHED THEN INSERT` must go
//! through full cross-engine index maintenance (surrogate + vector + FTS),
//! not just the raw document put — live, no restart.
