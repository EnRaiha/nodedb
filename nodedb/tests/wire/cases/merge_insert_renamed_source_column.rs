...
//! `MERGE ... WHEN NOT MATCHED THEN INSERT (cols) VALUES (s.exprs)` must
//! carry the real VALUES expressions across the bridge, not re-derive them
//! by name-matching target against source — a renamed source column
//! (`embedding` ← `s.new_embedding`) must not insert an empty value.
