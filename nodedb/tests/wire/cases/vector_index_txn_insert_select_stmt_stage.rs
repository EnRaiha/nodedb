...
//! In-transaction `INSERT ... SELECT` is resolved and staged at STATEMENT
//! time, not COMMIT time. Proven two ways: a later `SELECT` in the same
//! transaction sees the copied rows (read-your-own-writes), and after
//! COMMIT each copied row resolves through the target's vector index to its
//! own primary key (fresh, catalog-registered surrogate, not the source
//! row's).
