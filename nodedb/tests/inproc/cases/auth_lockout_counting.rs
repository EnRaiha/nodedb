...
fn state_with_lockout() -> (Arc<SharedState>, tempfile::TempDir) {
    ...
    let state = SharedState::new_with_credentials(dispatcher, wal, credentials, false).unwrap();
...
async fn pgwire_scram_correct_password_with_pending_change_does_not_count() {
    let server = nodedb_test_support::pgwire_harness::TestServer::start_password().await;
    server
        .shared
        .rate_limiter
