    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        match zerompk::from_msgpack::<Self>(data) {
            ...
            Err(zerompk::Error::ArrayLengthMismatch { actual, .. })
                if actual == super::legacy_entry::LegacyReplicatedEntry::FIELD_COUNT =>
            {
                zerompk::from_msgpack::<super::legacy_entry::LegacyReplicatedEntry>(data)
                    .ok()
                    .map(super::legacy_entry::LegacyReplicatedEntry::into_current)
            }
            Err(_) => None,
        }
    }
