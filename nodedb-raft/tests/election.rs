...
/// A node that voted for candidate A in term N, then crashes and
/// restarts, must NOT grant a vote to candidate B in the same term N. If HardState (current_term/voted_for) is lost across the
