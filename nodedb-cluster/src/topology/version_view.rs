...
//!
//! `SharedState::cluster_version_view()` (in the `nodedb` crate)
//! wraps a call to [`compute_from_topology`] reading
//! `cluster_topology` under a short read guard. Every call is
//! authoritative: adds, removes, and wire-version updates are
//! observed immediately because the topology `RwLock` is the single
//! source of truth.

use super::{CLUSTER_WIRE_FORMAT_VERSION, ClusterTopology, NodeInfo};
...
pub struct ClusterVersionView {
    ...
    /// Defaults to the local `CLUSTER_WIRE_FORMAT_VERSION` when the
...
impl ClusterVersionView {
    ...
    pub fn single_node() -> Self {
        Self {
            min_version: CLUSTER_WIRE_FORMAT_VERSION,
            max_version: CLUSTER_WIRE_FORMAT_VERSION,
            node_count: 0,
    ...
    pub fn all_upgraded(&self) -> bool {
        self.min_version == CLUSTER_WIRE_FORMAT_VERSION
    }
...
mod tests {
    use super::*;
    use super::{NodeInfo, CLUSTER_WIRE_FORMAT_VERSION};
    use crate::NodeState;
    ...
    fn empty_topology_falls_back_to_local() {
        ...
        assert_eq!(v.min_version, CLUSTER_WIRE_FORMAT_VERSION);
        assert_eq!(v.max_version, CLUSTER_WIRE_FORMAT_VERSION);
    ...
    fn single_version_cluster() {
        ...
        topo.add_node(node(1, CLUSTER_WIRE_FORMAT_VERSION));
        topo.add_node(node(2, CLUSTER_WIRE_FORMAT_VERSION));
        topo.add_node(node(3, CLUSTER_WIRE_FORMAT_VERSION));
        let v = compute_from_topology(&topo);
    ...
    fn mixed_version_n_minus_1() {
        ...
        topo.add_node(node(1, CLUSTER_WIRE_FORMAT_VERSION));
        topo.add_node(node(2, CLUSTER_WIRE_FORMAT_VERSION));
        topo.add_node(node(3, CLUSTER_WIRE_FORMAT_VERSION - 1));
        let v = compute_from_topology(&topo);
        assert!(v.is_mixed_version());
        assert!(!v.all_upgraded());
        assert!(v.is_supported_spread());
        assert_eq!(v.min_version, CLUSTER_WIRE_FORMAT_VERSION - 1);
    }
    ...
    fn unsupported_spread_detected() {
        let mut topo = ClusterTopology::new();
        topo.add_node(node(1, CLUSTER_WIRE_FORMAT_VERSION));
        topo.add_node(node(2, CLUSTER_WIRE_FORMAT_VERSION - 2));
        let v = compute_from_topology(&topo);
    ...
    /// With the window open (MIN < WIRE), a topology containing
    /// floor-v1 and current-v2 nodes must report `min_version == 1`
    /// and refuse to activate features gated on version 2.
    #[test]
    fn window_from_topology_with_floor_v1() {
        let mut topo = ClusterTopology::new();
        topo.add_node(node(1, 1));
        topo.add_node(node(2, CLUSTER_WIRE_FORMAT_VERSION));
        let v = compute_from_topology(&topo);
